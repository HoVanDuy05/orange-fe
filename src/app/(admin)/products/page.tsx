...
export default function ProductsPage() {
  ...
  const { data: rawProducts = [], isLoading: prodLoading, refetch, isRefetching } = useQuery({
    queryKey: ['products'],
    queryFn: async () => (await https.get('/products')).data?.data || []
  });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      notifications.show({ title: 'Thành công', message: editingProduct ? 'Đã cập nhật món ăn' : 'Đã thêm món mới', color: 'green' });
      handleClose();
    },
    onError: (error: any) => {
      ...
      <PageHeader 
        ...
        actions={
          <Group gap="sm">
            ...
            <Button leftSection={<Plus size={18} />} color="brand" onClick={open} radius="md" fw={800} size="sm">
               Thêm món ăn
            </Button>
          </Group>
        }
        ...
        <Group justify="space-between" wrap="nowrap">
          <Group gap="md" style={{ flex: 1 }}>
            ...
            <Select
              ...
              data={rawCategories.map((c: any) => ({ value: c.id.toString(), label: c.category_name }))}
            ...
            <Table.Td>
              <Group gap="md" wrap="nowrap">
                {p.image_url ? (
                  ...
                  <Center h={44} w={44} bg="gray.1" style={{ border: '1px solid #E2E8F0', color: '#CBD5E1', borderRadius: '12px' }}>
                    <ImageIcon size={20} />
                  </Center>
                )}
                <Box>
                  <Text size="sm" fw={800} style={{ color: '#1E293B' }}>{p.product_name}</Text>
            ...
            <Table.Td>
              <Stack gap={0}>
                {p.discount_price ? (
                  ...
                  <Text fw={900} size="sm" style={{ color: '#0F172A' }}>{formatCurrency(p.price)}</Text>
                )}
              </Stack>
            </Table.Td>
            <Table.Td>
              <Group gap={6}>
                 ...
                 <Text size="sm" fw={700} style={{ color: '#334155' }}>{p.sales_count || 0}</Text>
              ...
              <Group gap="xs" wrap="nowrap" justify="center">
                 ...
                 <Tooltip label="Xoá">
                   <ActionIcon variant="light" color="red" radius="md" onClick={() => modals.openConfirmModal({
                      ...
                      children: <Text size="sm">Món ăn này sẽ bị xoá khỏi thực đơn. Các đơn hàng cũ vẫn sẽ giữ lại thông tin món.</Text>,
                      labels: { confirm: 'Đồng ý xoá', cancel: 'Bỏ qua' },
                   ...
                   <Stack gap="md">
                      <SimpleGrid cols={2} spacing="md">
                        ...
                        <Select 
                          ...
                          data={rawCategories.map((c: any) => ({ value: c.id.toString(), label: c.category_name }))} 
                          required value={categoryId} onChange={setCategoryId} 
                          radius="md"
                          leftSection={<Layers size={16} color="#94A3B8" />}
                        />
                      </SimpleGrid>
